...
def build_process_spec(num_days: int = 365) -> "ProcessSpec":
    """Build the process specification for the factory simulation."""
    from plugboard.schemas import ProcessArgsSpec, ProcessSpec

    components: list[dict[str, _t.Any]] = [
        {
            "type": "factory_simulation.Iterator",
            "args": {"name": "clock", "num_days": num_days},
        },
    ]
    ...
    for i, cfg in enumerate(MACHINE_CONFIGS, start=1):
        ...
        connectors.append({"source": "clock.day", "target": f"machine_{i}.day"})
        connectors.append({"source": f"machine_{i}.daily_value", "target": f"factory.value_{i}"})
    ...
    return ProcessSpec(
        args=ProcessArgsSpec(components=components, connectors=connectors),
...
if __name__ == "__main__":
    ...
    async def run_single() -> None:
        ...
        process = ProcessBuilder.build(spec=spec)
        assert isinstance(process, LocalProcess)
        async with process:
            await process.run()
        # Retrieve final total value from the factory component
    ...
    def run_optimisation() -> None:
        ...
        print(
            f"  Maximum total value: ${result.metrics['component.factory.field.total_value']:,.0f}"
        )

    # Run single simulation
    print("Running single simulation with default maintenance intervals (30 days)...")
    asyncio.run(run_single())

    # Uncomment to run optimisation (requires Ray):
